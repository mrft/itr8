...
describe("operators/general/split.ts", () => {
  it("split(...) operator works properly", async () => {
    ...
    // sync
    assert.deepEqual(itr8ToArray(split("!")(itr8FromArray(input))), [
      ["Hello", "World"],
      ["Goodbye", "Space"],
      [],
    ]);

    assert.deepEqual(itr8ToArray(split("Hello")(itr8FromArray(input))), [
      [],
      ["World", "!", "Goodbye", "Space", "!"],
    ]);

    // async
    assert.deepEqual(await itr8ToArray(split("!")(itr8FromArrayAsync(input))), [
      ["Hello", "World"],
      ["Goodbye", "Space"],
      [],
    ]);
