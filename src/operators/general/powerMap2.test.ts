 ...
 *
 * Used in the powerMap2 tests cases, and maybe some other places...
 */
const transIts = {
  opr8Map: <TIn = unknown, TOut = unknown>(mapFn: (TIn) => TOut) => {
    return powerMap2(
      (nextIn, _state) => {
        ...
        return { next: { done: false, value: mapFn(nextIn.value) } };
      },
      () => undefined,
    );
  },
  ...
  /**
   * In order to compare with the old powerMap implementation
   * @param offset
   * @returns
   */
  opr8SkipPowerMap: (offset: number) => {
    return powerMap(
      (nextIn, state) => {
        if (nextIn.done) {
          return nextIn;
        ...
        return nextIn as TNextFnResult<unknown, number>;
      },
      () => 0,
    );
  },
   ...
   * @returns
   */
  opr8SkipPowerMapStateless: <TIn>(offset: number) => {
    let skipped = 0;
    return powerMapStateless<TIn, TIn>((nextIn) => {
      if (skipped < offset) {
        skipped++;
        return null;
      }
  ...
  opr8MapSyncAsync: (mapFn: (unknown) => unknown) => {
    return powerMap2(
      (nextIn, _state) => {
        ...
        return {
          next: { done: false, value: Promise.resolve(mapFn(nextIn.value)) },
        };
      },
      () => null,
    );
  ...
  opr8RepeatEachSyncSync: (count: number) => {
    return powerMap2<number, any, void>(
      (nextIn, _state) => {
        if (nextIn.done) {
          return { next: nextIn };
        }
        return {
          nextIterable: (function* () {
  ...
  opr8FilterSyncSync: <TIn>(filterFn: (TIn) => boolean) => {
    return powerMap2<TIn, TIn, void>(
      (nextIn, _state) => {
        if (nextIn.done || filterFn(nextIn.value)) {
          return { next: nextIn };
        }
        return { isLast: false } as TNextFnResult2<TIn, void>;
      },
      () => null,
    );
  },
  ...
  opr8TakeUseStateFactoryParams: (initVal: number) => {
    return powerMap2<number, any, number>(
      (nextIn, state: number) => {
        if (nextIn.done || state <= 0) {
          return { next: { done: true, value: undefined } };
        }
        return { next: nextIn, state: state - 1 };
      },
      () => initVal,
    );
  },
  ...
  opr8UseMultipleParams: (param1: number, param2: string, param3: boolean) => {
    return powerMap2<unknown, [unknown, number, string, boolean], boolean>(
      ...
      (
        nextIn,
        state: boolean,
      ): TNextFnResult2<[unknown, number, string, boolean], boolean> => {
        if (nextIn.done) {
          return { next: nextIn };
        }
        return {
  ...
  opr8TransNextFn: (
    transNextFn: (
      input: TNextFnResult2<unknown, undefined>,
    ) => TNextFnResult2<unknown, undefined>,
  ) => {
    return powerMap2<unknown, unknown, unknown>(
      (nextIn, _state) =>
        transNextFn(nextIn as TNextFnResult2<unknown, undefined>),
      () => undefined,
    );
  },
  /**
...
describe("./operators/general/powerMap2.ts", () => {
  describe("powerMap2(...) works properly", () => {
    // TODO: test ALL cases:
    ...
    describe("performance", () => {
      it("powerMapStateless is faster than powerMap", () => {
        const nrOfItems = 5_000_000;
        const startPowerMapStateless = Date.now();
        const resultPowerMapStateless = pipe(
          itr8Range(1, nrOfItems),
          transIts.opr8SkipPowerMapStateless(nrOfItems / 2),
          itr8ToArray,
        ) as number[];
        const durationPowerMapStateless = Date.now() - startPowerMapStateless;
        console.log(
          ` * powerMapStateless took ${durationPowerMapStateless} ms`,
        );

        const startPowerMap = Date.now();
        const resultPowerMap = pipe(
          itr8Range(1, nrOfItems),
          transIts.opr8SkipPowerMap(nrOfItems / 2),
          itr8ToArray,
        ) as number[];
        const durationPowerMap = Date.now() - startPowerMap;
        console.log(` * powerMap took ${durationPowerMap} ms`);

        assert.equal(resultPowerMapStateless.length, resultPowerMap.length);
        assert.deepEqual(resultPowerMapStateless, resultPowerMap);
        assert.isBelow(
          durationPowerMapStateless,
          durationPowerMap,
          "I was hoping powerMapStateless would be faster than powerMap",
        );
      }).timeout(10_000);
    });
     ...
     */
    describe.skip("allow composing of operators (more like transducers)", () => {
      it("all operators created with powerMap2 have a working transNextFn", () => {
