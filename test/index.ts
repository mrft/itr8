...
describe('itr8 test suite', () => {
  ...
  describe('our own \'operators\'', () => {
    ...
    it('limit(...) operator works properly', async () => {
      // sync
      assert.deepEqual(
        itr8ToArray(limit(5)(itr8Range(1, 7) as Iterator<number>)),
        [1, 2, 3, 4, 5],
      );

      assert.deepEqual(
        itr8ToArray(limit(5)(itr8Range(1, 3) as Iterator<number>)),
        [1, 2, 3],
      ...
      // async
      assert.deepEqual(
        await itr8ToArray(limit(5)(itr8RangeAsync(1, 7) as AsyncIterator<number>)),
        [1, 2, 3, 4, 5],
      );

      assert.deepEqual(
        await itr8ToArray(limit(5)(itr8RangeAsync(1, 3) as AsyncIterator<number>)),
        [1, 2, 3],
        'limit should return the entire input when the limit is set higher than the total nr of elements in the input',
      );
    });
    ...
    it('prefetch(...) operator works properly', async () => {
      const iteratorFactory = () => {
        ...
        return itr8Proxy(innerIteratorFactory());
      }
  ...
  describe('Check a really large set', () => {
    it('when running over a really large set, we should not run out of memory', async () => {
      ...
      const itSync = itr8Proxy(syncGen())
        .pipe(
          limit(1_000_000),
      ...
      const itAsync = itr8Proxy(asyncGen())
        .pipe(
          limit(1_000_000),
          map((x) => x.startsWith('nine') ? `9: ${x}` : x),
          groupPer(10),
