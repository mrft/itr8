...
describe('itr8 test suite', () => {
  ...
  describe('our own \'operators\'', () => {
    ...
    it('runningReduce(...) operator works properly', async () => {
      assert.deepEqual(
        itr8.itr8ToArray(itr8.itr8Range(0, 4).pipe(
          itr8.runningReduce({ reducer: (acc: number, cur: number) => acc + cur, initialValue: 0 }),
        )),
        [0, 1, 3, 6, 10],
      );

      assert.deepEqual(
        itr8.itr8Range(10, 15).pipe(
          // implement a simple 'count' by returning index + 1
          itr8.runningReduce({ reducer: (acc: number, cur: number, index: number) => index + 1, initialValue: undefined }),
          itr8.itr8ToArray,
        ),
        [1, 2, 3, 4, 5, 6],
      );
    ...
    it('dedup(...) operator works properly', async () => {
      ...
      assert.deepEqual(
        itr8.itr8FromArray(['one', 'two', 'three', 'four', 'five', 'six', 'seven', 'eight', 'nine', 'ten'])
        .pipe(
          itr8.dedup((v) => v.length),
          itr8.itr8ToArray,
        ),
